...
namespace XIVSlothComboPlugin.Combos
{
    ...
    internal class SamuraiYukikazeCombo : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID == SAM.Yukikaze)
            {
                ...
                {
                    if (CanWeave(actionID) && gauge.Kenki <= 50 && IsOffCooldown(SAM.Ikishoten) &&
                        level >= SAM.Levels.Ikishoten)
                        return SAM.Ikishoten;
                }

                if (((IsEnabled(CustomComboPreset.SamuraiOvercapFeature75) ? 75 : 0) |
                     (IsEnabled(CustomComboPreset.SamuraiOvercapFeature85) ? 85 : 0)) is var cap && cap != 0 &&
                    gauge.Kenki >= cap && CanWeave(actionID))
                {
    ...
    internal class SamuraiMangetsuCombo : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID == SAM.Mangetsu)
            {
                ...
                if (CanWeave(actionID))
                {
                    if (IsEnabled(CustomComboPreset.SamuraiIkishotenonmaincombo) && gauge.Kenki <= 50 &&
                        IsOffCooldown(SAM.Ikishoten) && level >= SAM.Levels.Ikishoten)
                        return SAM.Ikishoten;
    ...
    internal class SamuraiTsubameGaeshiShohaFeature : CustomCombo
    {
        protected internal override CustomComboPreset Preset { get; } =
            CustomComboPreset.SamuraiTsubameGaeshiShohaFeature;
    ...
    {
        protected internal override CustomComboPreset Preset { get; } =
            CustomComboPreset.SamuraiTsubameGaeshiIaijutsuFeature;
    ...
    {
        protected internal override CustomComboPreset Preset { get; } =
            CustomComboPreset.SamuraiIaijutsuTsubameGaeshiFeature;
    ...
    internal class SamuraiFeintProtection : CustomCombo
    {
        protected internal override CustomComboPreset Preset { get; } = CustomComboPreset.SamuraiFeintProtection;

        protected override uint Invoke(uint actionID, uint lastComboActionID, float comboTime, byte level)
        {
            if (actionID is SAM.Feint)
            {
                if (TargetHasEffect(SAM.Debuffs.Feint) && IsOffCooldown(actionID: SAM.Feint))
                    return NIN.Rabbit;
            }
